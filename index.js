...
            `
    ...
    const payload = {
        messages,
        temperature: 0 // <-- THE ONLY CHANGE IS HERE
    };
    try {
        const url = '/ask'
        ...
        if (!data.content || !data.content.trim()) {
            console.warn('Empty content in worker response:', data)
            renderReport('The AI did not return any content. Please try again.')
        } else {
